...
test.skip("Cofirm you can select only one radio button",async({page}) => {
    ...
    await expect(no).not.toBeChecked();     //assertion for  no radio button is shouldnot checked
...
test.skip("Confirm last field is disabled",async({page}) => {
    ...
    const maybe    = page.locator(`//input[@id="going"]`);
...
test("Find if the checkbox is selected?",  async({page}) => {
    ...
    const rememberMe = page.locator(`//label[contains(text()," Remember me ")]/input`);
    await rememberMe.check();

    await expect(rememberMe).toBeChecked();

    const termAccept = page.locator(`//label[contains(text()," I agree to the ")]/input`);
    await termAccept.check();
    await expect(termAccept).toBeChecked();
})